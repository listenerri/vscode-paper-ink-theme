...
| keyword | `#aa3606` | `#bc795b` | 关键字（ink 用更深的 oxide，派生） |
| muted | `#635147` | `#b0a290` | 弱化文字（行号、面包屑、注释） |
| fill | `#ebe4d2` | `#312620` | 面板、终端、建议框底 |
| hover | `#e1d8c3` | `#41332a` | 列表 / 页签悬停（比 fill 更深/浅一档，派生） |
| deep | — | `#1e1008` | 深色状态栏底（对应浅色的墨色状态栏） |
| rule | `#30231b40` | `#ece8d940` | 分隔线（前景色 25% 透明度） |
| moss | `#3e7050` | `#83a58e` | 字符串（派生） |
| slate | `#4d6291` | `#94a2be` | 函数 / 类型（派生） |
| ochre | `#8f6f23` | `#bba579` | 常量 / 属性 / 数字（派生） |
| brick | `#c13c3b` | `#cc7c76` | 错误（派生） |
...
- 注释为 muted 色 + 斜体
- 深色主题的语法色额外降彩度、压明度（v3）：亮底上“低彩度＝沉静”，
  暗底上同样的彩度会变成霓虹感；ink 的关键字使用独立于 UI 点缀的更深 oxide
